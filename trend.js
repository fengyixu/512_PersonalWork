...
var svg = d3.select("trendvis").append("svg")
...
var xScale = d3.date.scale()
    .range([0, width]),

    xScale2 = d3.date.scale()
...
var color = d3.scale.ordinal().range(["#48A36D", "#80CCB3", "#9788CD", "#CE80B0", "#E16167", "#E39158", "#DDC05E", "#F2DE8A"]);  
...
d3.csv("https://raw.githubusercontent.com/yuew03/house_compass/master/dataset/data_set/city_date_price_Monthly.csv.csv", function(error, data) { 
  color.doa2wszmain(d3.keys(data[0]).filter(function(key) { // Set the domain of the color ordinal scale to be all the csv headers except "date", matching a color to an issue
    return key !== "date"; 
  }));
